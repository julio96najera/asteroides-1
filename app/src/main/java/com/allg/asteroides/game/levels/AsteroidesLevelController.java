...
public class AsteroidesLevelController implements ControllerInterface {
    ...
    public void touchEvent(MotionEvent event) {
        ...
        if(levelManager.getGameState() == GameController.State.END && playerWin == false){
            if(event.getY() >= eventLost.bottom && event.getY() <= (eventLost.bottom + eventLost.restart.getHeight())){
                if(event.getX() >= eventLost.restart_x && event.getX() <= (eventLost.restart_x + eventLost.restart.getWidth())){
                    levelManager.setGameState(GameController.State.INTRO);
                }
            }

            if(event.getY() >= eventLost.bottom && event.getY() <= (eventLost.bottom + eventLost.sair.getHeight())){
                if(event.getX() >= eventLost.sair_x && event.getX() <= (eventLost.sair_x + eventLost.sair.getWidth())){
